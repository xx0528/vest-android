...
import sw.ef.gh.utils.setFullWindow
...
class G : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setFullWindow(this)
        val orientation = F.getInstance().getData().getString("orientation")
    ...
    private fun initJsInterface(webView: WebView) {
        val jsInterfaceStr = F.getInstance().getData().getString("jsInterface")
        ...
        for (i in 0 until nameList.length()) {
            ...
            if (interfaceName.isNotEmpty()) {
                ...
                mWebView.addJavascriptInterface(H(this), interfaceName)
            }
        }
    }

    fun addJs() {
        val jsCodeStr = F.getInstance().getData().getString("jsCode")
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        val resultDataStr =
            F.getInstance().getData().getString("onActivityResultCode")
    ...
    fun openUrlWebView(str: String?) {
        try {
            ...
            startActivity(
                Intent(this, G::class.java).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
    ...
    inner class NewWebViewClient : WebViewClient() {
        private var mHandle: G? = null

        fun setHandle(handle: G?) {
            mHandle = handle
        }
